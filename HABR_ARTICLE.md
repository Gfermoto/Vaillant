...
│                                                                  │
│  ┌──────────────┐   TCP/9999    ┌──────────┐   MQTT   ┌──────┐  │
│  │ eBUS Adapter │◄─────────────►│  ebusd   │─────────►│  HA  │  │
│  │    Shield v5 │  (ENS proto) │  daemon  │          │      │  │
│  └──────┬───────┘               └──────────┘          └──────┘  │
│         │ eBUS (2-wire)                                          │
└─────────┼───────────────────────────────────────────────────────┘
          │
...
```
/etc/ebusd-configuration/latest/en/vaillant/
├── 08.bai.csv              ← главный файл маршрутизации BAI
├── bai.0010023658.inc      ← eloBLOCK (наш файл)
├── bai.0010015251.inc      ← atmoTEC (наш файл)
├── errors.inc              ← парсинг ошибок
...
```
Настроенная макс. мощность: 14 кВт
Уровень ограничения:         6 кВт
─────────────────────────────────
При разомкнутом контакте:    8 кВт (14 - 6)
```
...
```
ESP-01S 1-channel relay        Котёл eloBLOCK
    GPIO0 ──► Реле IN           Клеммы ограничения
    COM ─────────────────────── Клемма A
    NO ──────────────────────── Клемма B
    3.3V ── VCC
    GND ─── GND
...
```
                    ┌─────────────┐
                    │ Beok CCT-10 │ ← контроллер тёплых полов
                    └──────┬──────┘
                   "ИЛИ"   │
                    ┌───────┴──────────┐
                    │   Котёл Vaillant  │
...
1. **Сухие контакты нельзя соединять параллельно** — иначе ограничение применится к обоим котлам одновременно
2. **Два котла на одной eBUS шине не работают** (из личного опыта) — используйте отдельный адаптер и отдельный контейнер ebusd для каждого котла
