...
С **дополнения 26.1+** изменилась схема настроек (`commandline_options`, каталог `/addon_configs/…`, локальный `--configpath`, ebusd 24+ и загрузка CSV по URL). Без локального набора файлов из [ebusd-configuration](https://github.com/john30/ebusd-configuration) и правильного пути к каталогу `en` возможны перезапуски и «неизвестные» сущности в MQTT. Пошагово: [HA_ADDON_EBUSD_26.md](https://github.com/Gfermoto/Vaillant/blob/main/HA_ADDON_EBUSD_26.md); пример опций: [ebusd-addon-26.example.txt](https://github.com/Gfermoto/Vaillant/blob/main/ebusd-addon-26.example.txt). Обновлённый [mqtt-hassio.cfg](https://github.com/Gfermoto/Vaillant/blob/main/mqtt-hassio.cfg): short/long JSON для Discovery, **`definition-retain`**, в `.inc` — приоритет **`r1`** на критичных параметрах и **HcPumpMode** `3=intermittent` ([issue #5](https://github.com/Gfermoto/Vaillant/issues/5)). В репозитории также [bai.308523.inc](https://github.com/Gfermoto/Vaillant/blob/main/bai.308523.inc) для fallback.
