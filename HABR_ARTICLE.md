...
# Текущая мощность (кВт, реальное время) — тип HEX:2 по аналогии с HW7503, не верифицирован
r,,CurrentPower,d.108 Power now,,,,E601,,,HEX:2,,kW,Current power output
```
...
| `CurrentPower` | E601 | HEX:2 | Текущая мощность | ⚠️ Тип не верифицирован (по аналогии HW7503) |
...
- Адреса eBUS для параметров **D.152** (фаза ограничения) и **D.153** (значение ограничения) — найдены в официальном мануале, но регистры eBUS **не найдены ни в одном публичном источнике**. Если их найти, можно управлять мощностью **без физического реле**!
- Формат параметра `CurrentPower` (E601) — применён `HEX:2` по аналогии с похожей моделью HW7503 ([issue #254](https://github.com/john30/ebusd-configuration/issues/254)), **не верифицирован на 0010023658**
- Параметры `ElementHours` (C401) — тип `hoursum2` по аналогии с газовыми котлами, **не верифицирован**
...
Электрические параметры (d.100–d.108, eloBLOCK specific):
  ...
  d.108 CurrentPower   E601  HEX:2    Текущая мощность (кВт) — тип не верифицирован
